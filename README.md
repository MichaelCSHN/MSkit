...
| 场景 | 三方结构 | 说明 | 优先级 |
|------|----------|------|--------|
| 野外搜救 | 组织 / 搜索 | 组织方负责搜救调度和定位态势；搜索方执行搜索、目标检测/变化检测和证据提交；无防护方 | P0 |
| Hide and Seek 实景对抗赛 | 组织 / 搜索 / 防护 | 组织方组织与裁判；搜索方搜索发现；防护方隐藏、防护和覆盖设计 | P0 |
| 大型 Milsim 实景游戏 | 组织 / 搜索 / 防护 | 组织方主办和安全裁判；搜索方搜索/推进；防护方防护/值守；仅限民用模拟和赛事组织 | P1 |
| 安保训练 | 组织 / 搜索 / 防护 | 组织方为教官/评估员；搜索方巡检发现异常；防护方设计防护区和巡查路线 | P1 |
| 灾害应急演练 | 组织 / 搜索 / 防护 | 组织方组织演练；搜索方搜索受困点/风险点；防护方维护安全区和物资区 | P2 |
...
1. **MVP 场景工作流**：`MSkit_v1.3.2_MVP_Scenario_Workflows.md`
2. **MVP PLAN**：`MSkit_v1.3.2_MVP_Development_Plan.md`
3. **MVP BOM**：`MSkit_v1.3.2_MVP_BOM_Cost_Table.md`
4. **MVP CHECKLIST**：`MSkit_v1.3.2_MVP_Development_Checklist.md`

`MVP_Scenario_Workflows.md` 是搜救和 Hide and Seek 任务动作、数据对象、路径规划模式、无人机模拟、搜索区更新、胜负/完成条件的权威源。正式工程版 `Development_Plan`、`BOM_Cost_Table`、`Development_Checklist` 保留为 **M1 之后的工程样机 / Pilot Batch / 量产路线参考**，不作为 M0 路演阶段的主执行文档。
...
| MVP 演示 | 搜救：组织方划区 + 搜索方无人机扫描/可疑点/路径重规划；Hide and Seek：组织方建场 + 搜索方检测 + 防护方覆盖规划；不做飞控控制 |
...
| 文档 | 说明 |
|------|------|
| [MSkit_v1.3.2_MVP_Scenario_Workflows.md](docs/MSkit_v1.3.2_MVP_Scenario_Workflows.md) | **MVP 场景工作流（现阶段优先，任务流程权威源）**：完整定义搜救和 Hide and Seek 的组织方/搜索方/防护方动作、数据对象、路径规划模式、无人机模拟、搜索区更新和完成/胜负条件 |
| [MSkit_v1.3.2_MVP_Development_Plan.md](docs/MSkit_v1.3.2_MVP_Development_Plan.md) | **MVP PLAN（现阶段优先，6 周计划/WBS/验收权威源）**：M0 路演执行计划，聚焦搜救与 Hide and Seek 工作流、组织方定位态势、搜索方目标检测/变化检测、防护方覆盖规划 |
...
| [MSkit_v1.3.2_Tri_Party_Functions.md](docs/MSkit_v1.3.2_Tri_Party_Functions.md) | **组织/搜索/防护三方架构与功能矩阵**：明确组织方、搜索方、防护方的通用功能、专有功能、权限边界和 MVP 展示取舍 |
| [MSkit_v1.3.2_MVP_Demo_Plan.md](docs/MSkit_v1.3.2_MVP_Demo_Plan.md) | **MVP 演示方案**：用于立项/融资路演，说明演示脚本、真实 vs 模拟披露和路演表达 |
...
> **规格来源约定（分层权威）**：M0 阶段以 MVP 场景工作流 / MVP PLAN / MVP BOM / MVP CHECKLIST 为执行主文档；M1 之后以正式工程 PLAN / BOM / CHECKLIST 为工程主文档。v1.3.2 文档集是 v1.3 的**变更集/覆盖层**，二者配套使用。
...
| 阶段 | 时间 | 主文档 | 交付物 |
|------|------|--------|--------|
| M0 MVP 路演演示 | 6 周 | MVP 场景工作流 / MVP PLAN / MVP BOM / MVP CHECKLIST | 搜救 + Hide and Seek 双场景 MVP：组织方定位态势/规划、搜索方目标检测/变化检测、防护方覆盖规划、报告导出、演示脚本、录屏备份 |
