...
> **基线**：`MSkit_v1.3.2_Spec.md`｜**用途**：替代历史 `MSkit_v1.3_Development_Checklist.md` 中与 v1.3.2 不一致的条目，并同步外协一级包 A–F 与 MVP 路演演示执行口径。
...
- [ ] GOD/COD/CCD 术语冻结并用于 UI、脚本和路演材料。
- [ ] 演示区域、离线地图、样例视频、图片、飞行日志准备完成。
- [ ] 数据接入：视频/图片/日志可导入，轨迹可上图。
- [ ] GOD：至少 1–3 类民用目标检测可稳定展示。
- [ ] COD：至少支持两组图像/视频片段变化高亮。
- [ ] CCD：可绘制任务区域、禁入区、起降点，并生成覆盖/采集建议。
- [ ] 定位-路径规划：可生成绕开禁入区的路径建议。
- [ ] 报告导出：可输出包含地图截图、检测摘要、变化点和路径建议的 HTML/PDF/Markdown 报告。
- [ ] 路演稳定性：连续演示 10 分钟无崩溃。
...
- [ ] README、Spec、MVP Demo Plan、BOM、Development Plan、Checklist、Outsourcing Modules 均同步 v1.3.2 口径。
