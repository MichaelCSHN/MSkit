...
- [ ] 三方通用功能确认：地图、PNT、任务状态、数据接入、事件标注、路径建议、报告导出、审计回放。
- [ ] 三方专有功能确认：黑方 PNT/全局规划，红方 GOD/COD，蓝方 CCD。
- [ ] GOD/COD/CCD/PNT 术语冻结并用于 UI、脚本和路演材料。
- [ ] 演示区域、离线地图、样例视频、图片、飞行日志准备完成。
- [ ] 数据接入：视频/图片/日志可导入，轨迹可上图。
- [ ] 黑方视图：活动区、红蓝区域、无人机轨迹、事件时间线、路径建议可显示。
- [ ] 红方 GOD：至少 1–3 类民用目标检测可稳定展示。
- [ ] 红方 COD：至少支持两组图像/视频片段变化高亮。
- [ ] 蓝方 CCD：可绘制保护区、禁入区、起降点，并生成覆盖/采集建议和盲区提示。
- [ ] 定位-路径规划：可生成绕开禁入区的红方搜索路径和蓝方巡查路径建议。
- [ ] 报告导出：可输出包含黑方总览、红方发现、蓝方方案和路径建议的 HTML/PDF/Markdown 报告。
- [ ] 路演稳定性：连续演示 10 分钟无崩溃。
