...
|------|------|
| 周期 | 6 周 |
| 主脚本 | Hide and Seek 实景对抗赛（黑/红/蓝） |
| 备选脚本 | 野外搜救（黑/红，可选蓝方安全区） |
| 可扩展说明 | 大型 Milsim 实景游戏、安保训练、灾害应急演练、越野定向/户外挑战赛、大型活动安保、工业/农林巡检、生态观察 |
| 硬件 | 方案 A：笔记本 + 无人机遥控器 + 无人机；方案 B：笔记本 + 无人机/离线素材导入 |
| 三方架构 | 黑方：组织者/裁判/调度；红方：攻方/搜救/搜索小队；蓝方：守方/安保/防护小队 |
| 核心功能 | 黑方 PNT/全局规划；红方 GOD/COD；蓝方 CCD；三方地图态势展示；报告导出 |
| 数据接入 | 视频、图片、飞行日志、预录素材；可选 HDMI/USB/RTSP/文件导入 |
| 合规边界 | 不下发飞控指令，不做自动起降/自动航线，不展示军事化目标分类或作战控制 |
| Go/No-Go | 10 分钟连续演示无崩溃；Hide and Seek 主脚本完整；野外搜救备选脚本核心流程可演示；3 分钟融资路演脚本和 8–10 分钟技术脚本完成 |
...
| D. Application Software | OP-07 + OP-08 | M3–M9 | FS Lead | 地图、任务、报告、Dashboard、视频 UI 链路；MVP 阶段承担笔记本 Web App、黑方 PNT、蓝方 CCD UI 主体 |
| E. AI Data & Model Optimization | OP-06 | M2–M10 | AI Lead | 数据集、mAP、模型 FPS、TensorRT engine；MVP 阶段承担红方 GOD/COD 演示模型 |
...
| 关口 | 月份 | Go 判据 |
|------|------|---------|
| G0 MVP Demo | 6 周 | Hide and Seek 主脚本可完整演示；野外搜救备选脚本核心流程可演示；黑方 PNT/规划、红方 GOD/COD、蓝方 CCD、三方报告可在笔记本上稳定运行；不含飞控控制 |
...
| WP | 修订内容 | Owner | 关联一级包 |
|----|----------|-------|------------|
| WP-0.1（新增） | MVP 三方路演演示：Hide and Seek 主脚本、野外搜救备选脚本、黑方 PNT/规划、红方 GOD/COD、蓝方 CCD、报告导出 | PM/FS/AI/GIS | D + E + 内部核心包 |
| WP-0.2（新增） | 黑/红/蓝角色、权限、视图和报告口径冻结 | PM/FS/LEG | D + 内部核心包 |
