...
> **基线**：`MSkit_v1.3.2_Spec.md`｜**状态**：内部评审稿｜**目的**：同步主规格中的 AI 测试口径、无人机数据接入边界、EMC Class A/B、环境测试声明、加密导出、BOM 地区化要求、外协一级包结构、黑/红/蓝三方架构和 MVP 路演演示计划。
...
| Go/No-Go | 10 分钟连续演示无崩溃；3 分钟融资路演脚本和 8–10 分钟技术脚本完成 |
...
| 关口 | 月份 | Go 判据 |
|------|------|---------|
| G0 MVP Demo | 6 周 | 黑方 PNT/规划、红方 GOD/COD、蓝方 CCD、三方报告可在笔记本上稳定演示；不含飞控控制；3 分钟和 10 分钟脚本完成 |
| G1 Spec 冻结 | M2 | v1.3.2 规格、BOM、Checklist、开发计划、外协一级包 SOW、三方角色口径一致；无 P0 文档冲突 |
...
| WP | 修订内容 | Owner | 关联一级包 |
|----|----------|-------|------------|
| WP-0.1（新增） | MVP 三方路演演示：黑方 PNT/规划、红方 GOD/COD、蓝方 CCD、报告导出 | PM/FS/AI/GIS | D + E + 内部核心包 |
