...
> **状态**：内部评审稿 / 外协规划建议  
> **目的**：将原 OP-01–OP-12 工作分解升级为更适合发包管理的“6 个一级外包包 + 1 个内部核心包”结构，同时保留 OP 作为二级 WBS、验收项和接口治理单元，并明确 MVP 路演阶段与正式外协包的关系。
...
| MVP 工作 | 建议归属 | 是否正式外协包 |
|----------|----------|----------------|
| 路演叙事、演示脚本、合规边界 | 内部核心包 | 否 |
| 笔记本 Web App、地图、报告、路径规划 | D 包能力的小范围原型支持 | 不建议签长期 D 包 SOW，可用短期 Sprint |
| GOD/COD 演示模型 | E 包能力的小范围原型支持 | 不建议签完整 E 包 SOW，可用短期模型/标注任务 |
| 无人机视频/图片/日志导入 | 内部 + C/D 能力支持 | 否，MVP 不做飞控 |
| 演示素材采集 | 内部或本地飞手/素材服务 | 否 |

MVP 阶段不得让外协方实现或展示无人机飞控控制、自动航线下发、军事化目标分类或作战控制能力。正式 A–F 外协包应在 M1 规格冻结后按 SOW 启动。
...
|--------|---------|----------|------------|--------------|
| A. Rugged Hardware & Mechatronics | OP-01 + OP-02 + OP-03 | 结构 CAD、装配、线束、电源、热设计、样件、环境预验证 | HW / ME | 是，M1 后 |
| B. RF / Wireless / Certification | OP-04 + OP-11 中认证技术部分 | 地区化无线 BOM、天线布局、RF 预扫描、认证资料清单 | HW / SCM / LEG | 是，M1 后 |
| C. Edge Platform & Device Firmware | OP-05 + OP-09 + OP-12 | Jetson 镜像、BSP、OTA、LoRa 服务、RTK/GNSS/IMU 融合服务 | EMB Lead | 是，M2 后 |
| D. Application Software | OP-07 + OP-08 | 地图、任务、报告、Dashboard、视频 UI、告警面板 | FS Lead | M3 后；MVP 可短期原型支持 |
| E. AI Data & Model Optimization | OP-06 | 数据集、训练脚本、模型权重、TensorRT engine、评估报告 | AI Lead | 是；MVP 可短期模型支持 |
| F. Manufacturing & Test Fixtures | OP-10 | 试产 SOP、治具、老化测试、出厂测试、良率报告 | QA / SCM | M8/M9 后启动 |
| 内部核心包 | 架构/安全/接口/验收/合规边界 | API schema、数据模型、验收脚本、CCB、最终签署、MVP 脚本 | PM / Architect / QA | 全程 |
...
| MVP 关系 | MVP 可先由 D 包能力实现笔记本 Web App、地图、CCD、路径规划和报告导出；正式 D 包 SOW 仍在 M3 后启动 |
...
| MVP 关系 | MVP 可先由 E 包能力实现 GOD/COD 演示模型；MVP 样例集不等于正式冻结测试集 |
...
| 指标 | 主责 | 协作 | 最终签署 |
|------|------|------|----------|
| MVP 路演演示稳定性 | 内部 Product / Demo Owner | D + E 小范围支持 | PM / QA |
...
| 阶段 | 内部主线 | 可并行外协 |
|------|----------|-------------|
| M0 / W1–W6 | MVP 路演演示、GOD/COD、CCD、定位-路径规划、报告导出、演示脚本 | D/E 能力小范围短期支持；不启动完整 A–F 长约 |
...
| 一级包 | 合同方式 | 付款建议 |
|--------|----------|----------|
| MVP 短期支持 | 2–6 周短 Sprint / 小额服务单 | 按演示功能验收；不绑定长期工程 SOW |
...
7. MVP 阶段的短期外协不得引入飞控控制、自动航线下发或军事化目标分类功能。
...
| 角色 | 人数 | 责任 |
|------|------|------|
| PM / Product Owner | 1 | 需求、分包管理、预算、里程碑、客户试点、MVP 路演脚本 |
| System Architect / Tech Lead | 1 | 架构、接口、技术取舍、集成问题裁决 |
| Embedded Lead | 1 | Jetson、BSP、驱动、系统服务、硬件联调 |
| Full-stack Lead | 1 | API、数据模型、Dashboard 集成、报告系统、MVP Web App |
| AI Lead | 1 | 数据、模型、评估、推理优化验收、GOD/COD 演示模型 |
| QA / Integration | 1 | 验收脚本、测试计划、缺陷追踪、出厂测试、端到端指标签署、MVP 稳定性验证 |
| Legal/Compliance | 外部顾问 + 内部 PM 对接 | 合规、许可证、用户协议、认证资料、MVP 路演用语审查 |
...
1. 内部 Product / Demo Owner 冻结路演脚本。
2. D 包能力小范围支持 Web App、地图、CCD、路径规划、报告。
3. E 包能力小范围支持 GOD/COD 演示模型。
...
MSkit 不应按 12 个 OP 直接外包，而应按 **6 个一级合同包 + 内部核心包** 管理；MVP 路演阶段应作为 6 周短 Sprint 由内部主导，只引入 D/E 能力的小范围支持。这样可以先证明融资叙事和核心功能闭环，再进入正式工程样机与外协发包。
